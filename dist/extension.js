...
const toolTreeDataProvider_1 = require("./providers/toolTreeDataProvider");
function activate(context) {
    ...
    const showPanelCommand = vscode.commands.registerCommand('mcpManager.showPanel', () => {
        mcpWebViewPanel_1.MCPWebViewPanel.createOrShow(context.extensionUri.fsPath, mcpManager);
    });
    ...
    const mcpToolsProvider = vscode.languages.registerCompletionItemProvider({ scheme: 'file', pattern: '**/{*.ts,*.js,*.json}' }, // Support for TypeScript, JavaScript and JSON files
    {
    ...
    const copilotProvider = vscode.languages.registerInlineCompletionItemProvider({ scheme: 'file', pattern: '**/{*.ts,*.js,*.json}' }, {
        async provideInlineCompletionItems(document, position, context, token) {
            ...
            const items = tools.map(tool => {
                const item = new vscode.InlineCompletionItem(`MCPTool.${tool.name}`, new vscode.Range(position, position));
                item.command = {
                    command: 'mcpManager.showToolDetails',
                    title: 'Show Tool Details',
                    arguments: [tool]
    ...
    const mcpHoverProvider = vscode.languages.registerHoverProvider({ scheme: 'file', language: '*' }, {
    ...
    const toolTreeDataProvider = new toolTreeDataProvider_1.ToolTreeDataProvider(mcpManager);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpManager.showToolDetails', (tool) => {
        const panel = vscode.window.createWebviewPanel('mcpToolDetails', `MCP Tool: ${tool.name}`, vscode.ViewColumn.One, {
            enableScripts: true
        });
        panel.webview.html = `
                <!DOCTYPE html>
        ...
        if (e.affectsConfiguration('mcp')) {
            ...
            if (servers) {
                Object.entries(servers).forEach(([serverId, config]) => {
                    ...
                    const completionDisposable = vscode.languages.registerCompletionItemProvider({ scheme: 'file' }, {
                        provideCompletionItems(document, position) {
                            const linePrefix = document.lineAt(position).text.substr(0, position.character);
    ...
    const mcpStatusProvider = vscode.languages.registerInlineCompletionItemProvider({ scheme: 'file' }, {
        async provideInlineCompletionItems(document, position) {
            ...
            const items = Object.keys(servers).map(serverId => {
                return new vscode.InlineCompletionItem(serverId, new vscode.Range(position, position));
            });
            return { items };
        }
    });
